...
router.get('/draft-orders/search', async (req, res, next) => {
  try {
    ...
    console.log(`[API] Searching for draft order #D${num}...`);
    
    // Search Shopify directly by name
    const result = await shopifyService.searchDraftByName(`#D${num}`);
