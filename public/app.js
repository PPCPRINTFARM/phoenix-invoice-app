 ...
 * Returns 30 most recent drafts, newest first
 */
router.get('/draft-orders', async (req, res, next) => {
  try {
    const { status = 'open' } = req.query;
    
    console.log(`[API] Fetching draft orders (status: ${status})`);
    
    // Shopify service now handles sorting and limiting to 30
    const result = await shopifyService.getDraftOrders({ status });
    
    const draftOrders = result.draft_orders || [];
    
    console.log(`[API] Returning ${draftOrders.length} draft orders`);
